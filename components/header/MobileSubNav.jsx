...
export default function MobileTabHeader({data, dataIndex, reverseFunction, leftValue, tabHeight}) {
  ...
  return (
    data?.map((item, index) => {
        var arra1 = item?.multiLink?.slice(0, 5);
        var arra2 = item?.multiLink?.slice(5, 8);
        ...
        return (
          <div key={index} className={`tabs ${Style.tabs} transition-all duration-700 ease-in-out ${dataIndex === index ? 'laptop-portrait:block xl-up:block z-[9] ipad:left-[0]' : 'laptop-portrait:hidden xl-up:hidden ipad:hidden z-0 ipad:!left-[-100%]'} ${subBack !== "xx" && leftValue ? 'ipad:!left-0 !opacity-[1]' : 'ipad:!left-[-100%] z-0 bg-transparent opacity-[0]'} ${tabHeight < 555 ? 'sm:h-full' : 'sm:h-[calc(100%+150px)]'} ipad:absolute ipad:top-0 ipad:w-full tablet:h-full tabletlarge:h-full phablet:h-full laptopsmall:h-full`}>
